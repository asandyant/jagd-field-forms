...
function summary() {
  ...
  const completedMemberPieces = repairs.reduce((sum,r)=>sum+completedPieces(r),0);
...
function memberUnits(r) {
  const raw = String(r.members || '').split(',').map(m => m.trim()).filter(Boolean);
  if (raw.length === 1 && raw[0].toUpperCase() === 'ALL') {
    return [{ key: 'ALL', label: 'ALL', units: Number(r.qtyMembers || 1) }];
  }
  return raw.map(m => ({ key: m, label: m, units: 1 }));
}

function completedMemberSet(r) {
  return new Set(Array.isArray(r.completedMembers) ? r.completedMembers.map(String) : []);
}

function completedPieces(r) {
  const done = completedMemberSet(r);
  return memberUnits(r).reduce((sum, m) => sum + (done.has(m.key) ? Number(m.units || 1) : 0), 0);
}

function memberProgressText(r) {
  return `${completedPieces(r)} of ${Number(r.qtyMembers || 0)} pieces complete`;
}

function memberBadgeButtons(r) {
  const done = completedMemberSet(r);
  return memberUnits(r).map(m => `
    <button class="member-chip ${done.has(m.key) ? 'member-done' : ''}" type="button" data-repair-id="${r.id}" data-member-key="${escapeHtml(m.key)}" title="Click to mark ${escapeHtml(m.label)} complete/incomplete">
      ${escapeHtml(m.label)}${m.units > 1 ? ` <small>${m.units} pcs</small>` : ''}
    </button>
  `).join('');
}
...
function renderKpis() {
  const s = summary();
  document.getElementById('completePercent').textContent = `${pct(s.completedMemberPieces, s.members)}%`;
  document.getElementById('completeCount').textContent = s.completedMemberPieces;
  document.getElementById('totalCount').textContent = s.members;
  document.getElementById('kpiGrid').innerHTML = `
    <div class="kpi-card"><span>Repair Locations</span><strong>${s.total}</strong></div>
    <div class="kpi-card"><span>Member Pieces Complete</span><strong>${s.completedMemberPieces} / ${s.members}</strong><em>${pct(s.completedMemberPieces, s.members)}%</em></div>
...
function renderChart() {
  ...
  (emergencyData.repairs || []).forEach(r => {
    ...
    groups[key].complete += completedPieces(r);
  ...
  const datasets = [
    ...
    { label: 'Member pieces complete', data: labels.map(l=>groups[l].complete), backgroundColor: '#16a34a' },
...
function renderRepairMatrix() {
  // The big clickable location-box matrix was removed. Field updates now happen by clicking the member chips in the Repair Locations list/detail panel.
}
    ...
    <article class="repair-card ${isAdditional(r) ? 'repair-additional' : 'repair-original'} ${Number(r.id) === Number(selectedRepairId) ? 'selected' : ''}" data-id="${r.id}">
      <div class="repair-top">
        <div>
          ...
          <div class="repair-meta">${memberProgressText(r)} · ${r.estimatedSf} est. SF · ${r.source}</div>
        </div>
        ${classPill(r)}<span class="status-pill ${statusClass}">${r.status}</span>
      </div>
      <div class="member-badges clickable-members">${memberBadgeButtons(r)}</div>
...
function renderDetail() {
  ...
  document.getElementById('detailGrid').innerHTML = `
    ...
    <div><span>Member Progress</span><strong>${memberProgressText(r)}</strong></div>
    ...
    <div class="detail-member-clicks"><span>Click members complete</span><div class="member-badges clickable-members">${memberBadgeButtons(r)}</div></div>
  `;
  bindMemberButtons();
...
function bindMemberButtons() {
  document.querySelectorAll('.member-chip').forEach(btn => {
    ...
    btn.addEventListener('click', event => {
      ...
      selectedRepairId = Number(repairId);
      toggleMember(repairId, memberKey).catch(err => alert(err.message));
    });
  });
}

async function toggleMember(repairId, memberKey) {
  const res = await fetch('/api/vn84b/emergency-steel-repairs/member', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id: repairId, memberKey, enteredBy: document.getElementById('enteredByInput')?.value || '' })
  });
  if (!res.ok) {
    const err = await res.json().catch(()=>({error:'Could not update member'}));
    throw new Error(err.error || 'Could not update member');
  }
  emergencyData = await res.json();
  renderFilters();
  render();
