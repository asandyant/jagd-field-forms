...
function localPdfAttachmentFiles(inputId){
  const stored=getPhotoFileStore(inputId);
  const fallback=[...(document.getElementById(inputId)?.files||[])];
  const files=stored.length ? stored : fallback;
  return files.filter(f=>/pdf$/i.test(f.name||'') || (f.type && f.type.includes('pdf'))).map(f=>({originalName:f.name, mimetype:f.type||'application/pdf', size:f.size||0}));
}
...
function photoInput(id,label='Photos / attached pages'){
  const pdfId = id + 'Pdf';
  return `<div class="evidenceUploadBox"><label for="${id}">${label}</label><input id="${id}" type="file" accept="image/*" multiple><p class="tiny"><b>Need more than one photo?</b> Take/select a photo, then tap Choose File again to add another. Image photos will print on attached photo pages.</p><div id="${id}Count" class="tiny"></div><div id="${id}Preview" class="photoGrid"></div><div class="pdfAttachBox"><label for="${pdfId}">PDF Attachments</label><p class="tiny">Use this for doctor notes, hospital paperwork, police reports, statements, or other PDF backup. These will print as an attachment list page with the report.</p><label class="btn light filePickBtn" for="${pdfId}">Attach PDF files</label><input id="${pdfId}" class="hiddenFileInput" type="file" accept="application/pdf,.pdf" multiple><div id="${pdfId}Count" class="tiny"></div><div id="${pdfId}Preview" class="photoGrid"></div></div></div>`;
}
...
function buildExtraPdfAttachmentPages(inputId, reportTitle, reportMeta=''){
  const files = localPdfAttachmentFiles(inputId);
  if(!files.length) return '';
  const rows = files.map((file,idx)=>`<tr><td>${idx+1}</td><td>${esc(file.originalName||'PDF attachment')}</td><td>${file.size?esc(Math.round(file.size/1024)+' KB'):''}</td></tr>`).join('');
  return `<div class="extraPrintSheet extraAttachmentSheet"><div class="extraPrintHeader"><img src="${logo}"><h1>${esc(reportTitle)} PDF ATTACHMENTS</h1></div><div class="extraPhotoMeta"><b>${esc(reportMeta)}</b><span>${files.length} PDF attachment${files.length===1?'':'s'}</span></div><table class="extraPrintTable"><tr><th>#</th><th>PDF File Name</th><th>Size</th></tr>${rows}</table><div class="notice"><b>Attachment note:</b> PDF files were selected with this report. Keep/email the listed PDFs with the saved report packet. Browser print cannot merge external PDFs directly into this generated report.</div></div>`;
}
...
function incidentReportForm(){
  ...
  setupOtherProject('irProject'); setupToday('irReportDate'); setupToday('irIncidentDate'); setupToday('irWitnessDate'); setupSecondWitnessToggle('irAddWitness2Btn','irWitness2Panel'); setupPhotoPreview('irPhotos'); setupPhotoPreview('irPhotosPdf'); initSignatureButtons(); extraPrintButton('irPrintBtn', buildIncidentPrint, 'irMsg', ()=>logGeneratedForm('ir', projectValue('irProject'), val('irReportDate'), `Incident Report - ${dateToDisplay(val('irReportDate'))} - ${cleanFilePart(projectValue('irProject'))}`));
}

function hasAnyField(ids){ return (ids||[]).some(id=>String(val(id)||'').trim()) || (ids||[]).some(id=>!!signatureStore[id]); }
function hasFirstIncidentWitness(){ return hasAnyField(['irWitnessName','irWitnessCompany','irWitnessPhone','irWitnessTime','irWitnessSupervisor','irWitnessStatement','irWitnessPrevent','irWitnessPrint','irWitnessSignature']); }
function hasSecondIncidentWitness(){ return hasAnyField(['irWitness2Name','irWitness2Company','irWitness2Phone','irWitness2Time','irWitness2Supervisor','irWitness2Statement','irWitness2Prevent','irWitness2Print','irWitness2Signature']); }
function hasFirstAccidentWitness(){ return hasAnyField(['harWitnessName','harWitnessCompany','harWitnessPhone','harWitnessTime','harWitnessSupervisor','harWitnessStatement','harWitnessUnusual','harWitnessPrevent','harWitnessPrint','harWitnessSignature']); }
function hasSecondAccidentWitness(){ return hasAnyField(['harWitness2Name','harWitness2Company','harWitness2Phone','harWitness2Time','harWitness2Supervisor','harWitness2Statement','harWitness2Unusual','harWitness2Prevent','harWitness2Print','harWitness2Signature']); }
function buildIncidentFirstWitnessPage(){
  if(!hasFirstIncidentWitness()) return '';
  return `<div class="extraPrintSheet irPrintSheet"><div class="extraPrintHeader"><img src="${logo}"><h1>INCIDENT REPORT - WITNESS STATEMENT</h1></div><table class="extraPrintTable"><tr><th>Project</th><td>${esc(projectValue('irProject'))}</td><th>Report Date</th><td>${esc(dateToSlashYYYY(val('irReportDate')))}</td></tr><tr><th>Incident Date</th><td>${esc(dateToSlashYYYY(val('irIncidentDate')))}</td><th>Employee</th><td>${esc(val('irEmployee'))}</td></tr></table><h2 class="extraSectionTitle">Witness Statement</h2><table class="extraPrintTable"><tr><th>Witness</th><td>${esc(val('irWitnessName'))}</td><th>Company / Trade</th><td>${esc(val('irWitnessCompany'))}</td><th>Phone</th><td>${esc(val('irWitnessPhone'))}</td></tr><tr><th>Date / Time</th><td>${esc(dateToSlashYYYY(val('irWitnessDate')))} ${esc(val('irWitnessTime'))}</td><th>Supervisor Notified</th><td colspan="3">${esc(val('irWitnessSupervisor'))}</td></tr></table>${extraPrintBox('Witness Statement - In Their Own Words',val('irWitnessStatement'),2.35)}${extraPrintBox('How Could This Be Prevented in the Future?',val('irWitnessPrevent'),1.0)}<div class="extraSigGrid two"><div><b>Witness Print Name:</b> ${esc(val('irWitnessPrint')||val('irWitnessName'))}</div><div><b>Witness Signature:</b> ${sigPrint(signatureStore.irWitnessSignature,'')}</div></div></div>`;
}
function buildIncidentSecondWitnessPage(){
  if(!hasSecondIncidentWitness()) return '';
  return `<div class="extraPrintSheet irPrintSheet"><div class="extraPrintHeader"><img src="${logo}"><h1>INCIDENT REPORT - SECOND WITNESS</h1></div><table class="extraPrintTable"><tr><th>Project</th><td>${esc(projectValue('irProject'))}</td><th>Report Date</th><td>${esc(dateToSlashYYYY(val('irReportDate')))}</td></tr><tr><th>Incident Date</th><td>${esc(dateToSlashYYYY(val('irIncidentDate')))}</td><th>Employee</th><td>${esc(val('irEmployee'))}</td></tr></table><h2 class="extraSectionTitle">Second Witness Statement</h2><table class="extraPrintTable"><tr><th>Witness 2</th><td>${esc(val('irWitness2Name'))}</td><th>Company / Trade</th><td>${esc(val('irWitness2Company'))}</td><th>Phone</th><td>${esc(val('irWitness2Phone'))}</td></tr><tr><th>Date / Time</th><td>${esc(dateToSlashYYYY(val('irWitness2Date')))} ${esc(val('irWitness2Time'))}</td><th>Supervisor Notified</th><td colspan="3">${esc(val('irWitness2Supervisor'))}</td></tr></table>${extraPrintBox('Second Witness Statement - In Their Own Words',val('irWitness2Statement'),2.35)}${extraPrintBox('How Could This Be Prevented in the Future?',val('irWitness2Prevent'),1.0)}<div class="extraSigGrid two"><div><b>Witness 2 Print Name:</b> ${esc(val('irWitness2Print')||val('irWitness2Name'))}</div><div><b>Witness 2 Signature:</b> ${sigPrint(signatureStore.irWitness2Signature,'')}</div></div></div>`;
}
function buildAccidentFirstWitnessPage(){
  if(!hasFirstAccidentWitness()) return '';
...
function buildAccidentSecondWitnessPage(){
  if(!hasSecondAccidentWitness()) return '';
...
function buildIncidentPrint(){
  ...
  const firstWitnessPage = buildIncidentFirstWitnessPage();
  ...
  const attachmentPages = buildExtraPdfAttachmentPages('irPhotosPdf', 'Incident Report', meta);
  document.title=`Incident Report - ${dateToDisplay(val('irReportDate'))} - ${cleanFilePart(projectValue('irProject'))}`; setPrint(html + firstWitnessPage + secondWitnessPage + photoPages + attachmentPages);
}
...
function heavyAccidentReportForm(){
  ...
  setupOtherProject('harProject'); setupToday('harDate'); setupToday('harReportDate'); setupToday('harWitnessDate'); setupSecondWitnessToggle('harAddWitness2Btn','harWitness2Panel'); setupPhotoPreview('harPhotos'); setupPhotoPreview('harPhotosPdf'); initSignatureButtons(); const d=document.getElementById('harDate'), day=document.getElementById('harDay'); const upd=()=>{if(d&&d.value&&day){day.value=new Date(d.value+'T00:00:00').toLocaleDateString(undefined,{weekday:'long'});}}; d&&d.addEventListener('change',upd); upd(); extraPrintButton('harPrintBtn', buildHeavyAccidentPrint, 'harMsg', ()=>logGeneratedForm('har', projectValue('harProject'), val('harReportDate'), `Accident Report - ${dateToDisplay(val('harReportDate'))} - ${cleanFilePart(projectValue('harProject'))}`));
}
function buildHeavyAccidentPrint(){
  ...
  const firstWitnessPage = buildAccidentFirstWitnessPage();
  ...
  const attachmentPages = buildExtraPdfAttachmentPages('harPhotosPdf', 'Accident Report', meta);
  document.title=`Accident Report - ${dateToDisplay(val('harReportDate'))} - ${cleanFilePart(projectValue('harProject'))}`; setPrint(html + firstWitnessPage + secondWitnessPage + photoPages + attachmentPages);
}
