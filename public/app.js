...
async function renderAdminCoa(editMat=null){
  ...
  try{
    ...
    c.innerHTML=`<div class="panel"><h2>COA / Material Library</h2><p class="tiny">Materials here feed the PIR COA helper. Add/edit manual records now. OCR upload/review is the next step after this.</p>${materialFormHtml(editMat||{})}<div class="adminToolbar"><select id="adminMatFilterProject"><option value="">All jobs</option>${adminProjectOptions('')}</select><input id="adminMatSearch" placeholder="Search product, batch, color"><span>${active.length} active / ${rows.length} total</span></div><div id="adminMatTable"></div></div>`;
    setupAdminMaterialForm();
    const renderTable=()=>{
      const q=val('adminMatSearch').toLowerCase(), project=val('adminMatFilterProject');
      const filtered=rows.filter(m=>(!project||m.project===project) && `${m.project||''} ${m.mfr||''} ${m.prodName||''} ${m.batch||''} ${m.color||''} ${m.component||''}`.toLowerCase().includes(q)).slice(0,250);
      document.getElementById('adminMatTable').innerHTML=`<div class="adminTableWrap"><table class="adminTable"><tr><th>Project</th><th>Component</th><th>Product</th><th>Batch</th><th>Exp</th><th></th></tr>${filtered.map(m=>`<tr class="${m.disabled?'mutedRow':''}"><td>${esc(m.project||'')}</td><td>${esc(m.component||'')}</td><td><b>${esc(m.prodName||m.description||'')}</b><br><small>${esc(m.color||'')} ${esc(m.mfr||'')}</small></td><td>${esc(m.batch||'')}</td><td>${esc(m.expDate||'')}</td><td><button class="btn small light" data-edit-mat="${esc(m.id)}" type="button">Edit</button> <button class="btn small danger" data-disable-mat="${esc(m.id)}" type="button">Disable</button></td></tr>`).join('')}</table></div>`;
      document.querySelectorAll('[data-edit-mat]').forEach(b=>b.onclick=()=>renderAdminCoa(rows.find(m=>String(m.id)===b.dataset.editMat)));
...
function setupAdminMaterialForm(){
  ...
  document.getElementById('adminMatClearBtn').onclick=()=>renderAdminCoa();
  save.onclick=async()=>{
    ...
    try{await adminFetch('/api/admin/materials',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); serverMaterialsLoaded=false; document.getElementById('adminMatMsg').innerHTML='<div class="notice success">Material saved. It is now available to the PIR helper for that job.</div>'; setTimeout(()=>renderAdminCoa(),700);}catch(e){document.getElementById('adminMatMsg').innerHTML=`<div class="notice">${esc(e.message)}</div>`;}
  };
}
