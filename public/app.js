...
function tmFieldView(){
  ...
  app.innerHTML=`<div class="container tmShell">
    ...
    <section id="tmFormPanel" class="panel tmPanel">
      <div class="tmTitle"><div><span class="formTag">No login required</span><h2>Submit Receipt / Cost</h2><p>Choose the job, add all pages, review them, and submit once.</p></div></div>
      <div class="tmGrid">
        ${selectField('tmProject','Project / Job *','<option value="">Loading jobs...</option>')}
        <div id="tmCustomBox" class="tmCustom hidden">${inputField('tmCustomContract','Contract number or job name *')}${inputField('tmCustomName','Job nickname / location')}</div>
        ${selectField('tmType','Record type *','<option>Receipt / Materials</option><option>Rental</option><option>JAGD-Owned Equipment</option><option>Other Support Document</option>')}
        ${inputField('tmVendor','Vendor / Company *')}
        ${inputField('tmDate','Purchase / service date *','date')}
        ${inputField('tmAmount','Total amount','number')}
        ${selectField('tmCategory','Category *','<option>Materials</option><option>Small Tools</option><option>Consumables</option><option>Fuel</option><option>Safety Supplies</option><option>Equipment Repair</option><option>Rental</option><option>Delivery / Freight</option><option>Warehouse Stock</option><option>Other</option>')}
        ${selectField('tmPayment','Paid with *','<option>Company AmEx</option><option>Company Credit Card</option><option>Personal Card - Reimbursement Needed</option><option>Cash</option><option>Vendor Account</option><option>Unknown</option>')}
        ${inputField('tmPurchaser','Purchased by')}
        ${inputField('tmSubmitter','Submitted by *')}
        <label class="full">What was purchased, rented, or used? *<textarea id="tmDescription" rows="3"></textarea></label>
      </div>
      <section id="tmRentalBox" class="tmSubPanel hidden"><h3>Rental Details</h3><div class="tmGrid">${inputField('tmRentalEquipment','Equipment *')}${inputField('tmRentalUnit','Unit / contract #')}${inputField('tmRentalStart','Start date','date')}${inputField('tmRentalEnd','Expected return date','date')}${selectField('tmRentalRateType','Rate type','<option>Daily</option><option>Weekly</option><option>Monthly</option>')}${inputField('tmRentalRate','Rate','number')}</div></section>
      <section id="tmOwnedBox" class="tmSubPanel hidden"><h3>JAGD-Owned Equipment</h3><div class="tmGrid">${inputField('tmEquipmentId','Equipment ID')}${inputField('tmEquipmentDescription','Equipment description')}${inputField('tmInUse','In-use hours','number')}${inputField('tmStandby','Standby hours','number')}${inputField('tmOwnershipRate','Ownership rate','number')}${inputField('tmOperatingRate','Operating rate','number')}</div></section>
      ...
      <div id="tmSubmitMsg"></div><button id="tmSubmitBtn" class="btn primary tmSubmitBtn">Submit Receipt / Cost</button>
  ...
  document.getElementById('tmType').onchange=e=>{document.getElementById('tmRentalBox').classList.toggle('hidden',e.target.value!=='Rental');document.getElementById('tmOwnedBox').classList.toggle('hidden',e.target.value!=='JAGD-Owned Equipment');};
...
async function tmSubmit(){
 ...
 const data={projectId,customContract:val('tmCustomContract'),customName:val('tmCustomName'),type:val('tmType'),vendor,transactionDate,amount:val('tmAmount'),category:val('tmCategory'),paymentMethod:val('tmPayment'),purchaser:val('tmPurchaser'),submitter,description,rental:val('tmType')==='Rental'?{equipment:val('tmRentalEquipment'),unit:val('tmRentalUnit'),startDate:val('tmRentalStart'),endDate:val('tmRentalEnd'),rateType:val('tmRentalRateType'),rate:Number(val('tmRentalRate')||0),open:true}:null,owned:val('tmType')==='JAGD-Owned Equipment'?{equipmentId:val('tmEquipmentId'),description:val('tmEquipmentDescription'),inUseHours:Number(val('tmInUse')||0),standbyHours:Number(val('tmStandby')||0),ownershipRate:Number(val('tmOwnershipRate')||0),operatingRate:Number(val('tmOperatingRate')||0)}:null};
...
function tmOfficeFilter(kind){let rows=window.tmOfficeRows||[];if(kind==='review')rows=rows.filter(r=>['New','Missing Information'].includes(r.status)||r.customJob);if(kind==='receipts')rows=rows.filter(r=>r.type==='Receipt / Materials');if(kind==='rentals')rows=rows.filter(r=>r.type==='Rental');document.getElementById('tmOfficeRows').innerHTML=tmOfficeRowsHtml(rows);}
