...
function pirIsGwbProject(){
  const p=projectValue('pirProject').toUpperCase();
  return p.includes('GWB') || p.includes('GEORGE WASHINGTON') || p.includes('GW ');
}
function pirMaterialOptions(componentFilter=''){
  const mats=GWB_PIR_MATERIALS.filter(m=>{
  ...
  return '<option value=""></option>' + Object.keys(groups).sort().map(g=>`<optgroup label="${esc(g)}">${groups[g].map(m=>`<option value="${esc(m.id)}">${esc(m.label)}</option>`).join('')}</optgroup>`).join('');
}
function pirMaterialSelect(i, slot, label, filter){
  return `<div class="pirMaterialControl"><label for="pirMaterial${slot}${i}">${label}</label><select id="pirMaterial${slot}${i}" class="pirMaterialSelect" data-mix="${i}" data-slot="${slot}">${pirMaterialOptions(filter)}</select></div>`;
}
...
function applyPirMaterialToMix(i, slot, matId){
  const mat=GWB_PIR_MATERIALS.find(m=>m.id===matId);
  if(!mat) return;
...
function updatePirMaterialVisibility(){
  const show=pirIsGwbProject();
  document.querySelectorAll('.pirMaterialNotice,.pirMaterialControl').forEach(el=>{el.style.display=show?'block':'none';});
}
...
function mixBlockForm(i){
  return `<div class="panel innerPanel mixBlock" data-mix="${i}"><h3>Mix / Application Block ${i}</h3><div class="notice pirMaterialNotice"><b>GWB COA helper:</b> Pick a material/batch below to auto-fill Batch, Mfg Date, Shelf Life/Exp, Mfr, Product, and Color. All fields remain editable.</div><div class="grid two pirMaterialControls">${pirMaterialSelect(i,'A','GWB COA Product / Part A','main')}${pirMaterialSelect(i,'B','GWB COA Hardener / Dust / Component','component')}</div><div class="grid four">${field('pirMixLoc'+i,'Location')}${field('pirMixTime'+i,'Time','time')}${selectField('pirMixWitness'+i,'Mix Witnessed and Acceptable',['','YES','NO','N/A'])}${field('pirBatchA'+i,'Batch # A')}${field('pirMfgA'+i,'A Mfg Date')}${field('pirShelfA'+i,'A Shelf Life')}${field('pirBatchB'+i,'Batch # B')}${field('pirMfgB'+i,'B Mfg Date')}${field('pirShelfB'+i,'B Shelf Life')}${field('pirDust'+i,'Dust')}${field('pirThinner'+i,'Thinner Type')}${field('pirVolume'+i,'% By Volume')}${field('pirMfr'+i,'Mfr')}${field('pirProd'+i,'Prod. Name')}${field('pirColor'+i,'Color')}${field('pirKit'+i,'Kit Sz/Cond.')}${field('pirPot'+i,'Pot Life')}${field('pirShelf'+i,'Shelf Life')}${field('pirInduction'+i,'Induction Time')}${field('pirTemp'+i,'Temperature')}${field('pirQty'+i,'Quantity Mixed')}${field('pirStart'+i,'Start')}${field('pirFinish'+i,'Finish / Stop')}${field('pirGallons'+i,'Total Gallons')}${field('pirSystem'+i,'Coat / System')}${field('pirMethod'+i,'Application Method')}${field('pirGunTip'+i,'Gun/Tip Size')}${field('pirElapsed'+i,'Time elapsed between coats')}${field('pirDFTPrev'+i,'DFT Avg. Previous Coat')}</div></div>`;
}
