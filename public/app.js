...
async function loadActiveWorkers(){
  ...
  try{
    ...
    if(res.ok){
      const json = await res.json();
      if(Array.isArray(json.rows) && json.rows.length) activeWorkers = json.rows;
    }
  ...
  try{
    if(!activeWorkers.length){
      ...
      if(res.ok){ const json = await res.json(); if(Array.isArray(json) && json.length) activeWorkers = json; }
    }
...
function buildHeavyAccidentPrint(){
  ...
  document.title=`Heavy Accident Report - ${dateToDisplay(val('harReportDate'))} - ${cleanFilePart(projectValue('harProject'))}`; setPrint(html);
}
