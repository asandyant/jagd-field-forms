...
async function renderAdminCoa(editMat=null, mode='list'){
  ...
  try{
    ...
    const renderTable=()=>{
      ...
      const filtered=rows.filter(m=>(!project||m.project===project) && `${m.project||''} ${m.mfr||''} ${m.prodName||''} ${m.batch||''} ${m.color||''} ${m.component||''} ${m.fileName||''}`.toLowerCase().includes(q)).slice(0,350);
      ...
      document.getElementById('adminMatTable').innerHTML=`<div class="adminTableWrap"><table class="adminTable"><tr><th>Project</th><th>Component</th><th>Product</th><th>Batch</th><th>Exp</th><th>Status</th><th></th></tr>${filtered.map(m=>`<tr class="${m.disabled?'mutedRow':''}"><td>${esc(m.project||'')}</td><td>${esc(m.component||'')}</td><td><b>${esc(m.prodName||m.description||'')}</b><br><small>${esc(m.color||'')} ${esc(m.mfr||'')} ${m.fileName?('File: '+esc(m.fileName)):''}</small></td><td>${esc(m.batch||'')}</td><td>${esc(m.expDate||'')}</td><td>${m.disabled?'Needs Review / Disabled':'Active'}</td><td><button class="btn small light" data-edit-mat="${esc(m.id)}" type="button">Edit</button> <button class="btn small danger" data-disable-mat="${esc(m.id)}" type="button">Disable</button></td></tr>`).join('')}</table></div>`;
      document.querySelectorAll('[data-edit-mat]').forEach(b=>b.onclick=()=>renderAdminCoa(rows.find(m=>String(m.id)===b.dataset.editMat),'manual'));
